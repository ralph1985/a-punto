...
export function evaluateTask(task: MaintenanceTaskSchedule, currentKm: number | null, now = new Date()) {
  const dueDate = task.fixedDueDate ?? (task.intervalMonths !== null && task.baselineDate ? addMonths(task.baselineDate, task.intervalMonths) : null);
  const dueKm = task.intervalKm !== null && task.baselineOdometerKm !== null ? task.baselineOdometerKm + task.intervalKm : null;
  const dateDelta = dueDate !== null ? Math.ceil((dueDate.getTime() - now.getTime()) / 86_400_000) : null;
  ...
  const needsOdometer = dueKm !== null && currentKm === null;
  const status: TaskStatus = overdue
    ...
    : soon
      ...
      : needsOdometer
        ? "needs-odometer"
        : dueDate !== null || dueKm !== null
          ? "upcoming"
          : "unscheduled";
  const reasons = [
    dateDelta !== null ? dateDelta < 0 ? `Vencida hace ${Math.abs(dateDelta)} días` : `En ${dateDelta} días` : null,
    kmDelta !== null ? kmDelta < 0 ? `Superada por ${Math.abs(kmDelta).toLocaleString("es-ES")} km` : `Faltan ${kmDelta.toLocaleString("es-ES")} km` : null,
    needsOdometer ? "Registra el odómetro para evaluar los kilómetros" : null,
