...
export function AppNavigation({ current, vehicles }: { current: "today" | VehicleRouteSlug; vehicles: Array<{ slug: string; name: string; menuIcon: VehicleMenuIcon }> }) {
  return <nav className="app-navigation" aria-label="Secciones principales"><Link className="wordmark" href="/" aria-label="A Punto, inicio"><span className="wordmark-icon"><BrandCar size={25} /></span><span>A Punto</span></Link><div className="navigation-items"><Link href="/" className={current === "today" ? "navigation-item active" : "navigation-item"}><Gauge size={20} weight={current === "today" ? "fill" : "regular"} aria-hidden="true" /><span>Hoy</span></Link>{vehicles.map((vehicle) => { const Icon = vehicleIcons[vehicle.menuIcon]; const active = current === vehicle.slug; return <Link key={vehicle.slug} href={`/${vehicle.slug}`} className={active ? "navigation-item active" : "navigation-item"}><Icon size={20} weight={active ? "fill" : "regular"} aria-hidden="true" /><span>{vehicle.name}</span></Link>; })}</div></nav>;
}
